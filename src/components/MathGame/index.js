...
const randomImage = Math.ceil(Math.random() * imagesList.length)

const thumbnail1 = imagesList[randomImage]

const gettingImage = () => {
  const itemUrl = Math.ceil(Math.random() * imagesList.length)
  const itemCont = imagesList[itemUrl]
...
class MathGame extends Component {
  ...
  tick = () => {
    ...
    if (timing === 0) {
      ...
      this.setState(prevState => ({timing: prevState.timing - 1}))
    }
  }
  ...
  checkingThumbnail = id => {
    const {containerImage, timing} = this.state
    console.log(timing)
    ...
    if (containerImage.id === id) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        containerImage: gettingImage(),
      }))
    } else {
      clearInterval(this.timerId)
      this.setState({scoreCard: true})
  ...
  reset = () => {
    this.setState({score: 0, timing: 60, scoreCard: false})
  }

  render() {
    const {category, score, timing, containerImage, scoreCard} = this.state
    ...
    if (scoreCard) {
      scoreCont = (
        <div className="score-cont">
          ...
          />
          <p className="heading2">YOUR SCORE</p>
          <h1 className="heading3">{score}</h1>
      ...
      scoreCont = (
        <div>
          <ul className="tab-cont">
            <li>
              <img src={containerImage.imageUrl} alt="match" className="img" />
            </li>
          </ul>
        ...
        <ul className="game-logo">
          ...
          <li className="cont1">
            ...
            <img
              ...
              className="timer-img"
            />
            <p className="time">{timing} sec</p>
          </li>
        </ul>

        <div className="inner-container">{scoreCont}</div>
      </div>
    )
  }
}
